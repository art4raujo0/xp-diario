...
router.post("/", autenticar, async (req, res) => {
  try {
    ...
    if (!nome) {
      return res.status(400).json({ erro: "Nome da materia e obrigatorio" });
    }

    const duplicada = await pool.query(
      `SELECT di_id
       FROM disciplina
       WHERE LOWER(TRIM(di_disciplina)) = LOWER(TRIM($1))
         AND (di_usuario_id = $2 OR di_usuario_id IS NULL)
       LIMIT 1`,
      [nome, usuarioId]
    );

    if (duplicada.rowCount > 0) {
      return res.status(409).json({ erro: "Materia ja cadastrada" });
    }
    ...
    const result = await pool.query(
      `INSERT INTO disciplina (di_disciplina, di_dificuldade, di_descricao, di_cor, di_usuario_id)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [nome, di_dificuldade || null, di_descricao || null, di_cor || null, usuarioId]
    );
    ...
    const result = await pool.query(
      `UPDATE disciplina
       SET di_disciplina = $1,
           ...
           di_descricao = $3,
           di_cor = $4,
           di_usuario_id = COALESCE(di_usuario_id, $6)
       WHERE di_id = $5 AND (di_usuario_id = $6 OR di_usuario_id IS NULL)
       RETURNING *`,
      [nome, di_dificuldade || null, di_descricao || null, di_cor || null, id, usuarioId]
    );
